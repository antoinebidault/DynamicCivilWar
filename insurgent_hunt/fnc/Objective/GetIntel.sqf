...
private _potentialIntel = [];
{
    if (_x select 2)then{
        {
            if (!(_x getVariable["IH_isIntelRevealed",false]) && _x getVariable["IH_isIntel",false] && _pos distance _x < 500)then{
...
_intel setVariable["IH_isIntelRevealed",true];
private _marker = createMarker [format["s%1",random 13100],getPos _intel];
_marker setMarkerShape "ICON";
_marker setMarkerColor "ColorBlack";
_marker setMarkerType "hd_objective";
_intel setVariable["IH_markerIntel",_marker];
_intel setVariable["IH_task",_taskId];
