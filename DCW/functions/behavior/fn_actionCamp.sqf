...
*/
 ...
 _this addAction [format["<t color='#00FF00'>%1 (1 hour)</t>", localize "STR_DCW_actionCamp_setUpAction"], {
    ...
    if((_unit findNearestEnemy _unit) distance _unit < 100) exitWith {
        hint localize "STR_DCW_actionCamp_error" ;
    };

    if(([getPos _unit, 1, getDir _unit ] call BIS_fnc_relPos) isFlatEmpty  [2, -1, 0.3, 2, 0, false, _unit] isEqualTo [] 
        && surfaceType (getPosATL _unit) find "forest" == -1 /* stigate-fr issue */ ) exitWith { 
